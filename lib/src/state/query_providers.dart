...
final eventByIdProvider = Provider.family<PlannerEvent?, String>((ref, id) {
...
final interactionsByContactProvider =
    Provider.family<List<CrmInteraction>, String>((ref, contactId) {
...
final filteredContactsProvider =
    Provider.family<List<Connection>, ContactFilter>((ref, filter) {
      final connections = ref.watch(
        appControllerProvider.select((state) => state.connections),
      );
      ...
      final filtered = connections.where((c) {
        final matchesQuery =
            ...
            '${c.name} ${c.email} ${c.category}'.toLowerCase().contains(

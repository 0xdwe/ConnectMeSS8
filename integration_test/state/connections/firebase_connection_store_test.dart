...
void main() {
  ...
  test('save writes all 14 fields including schemaVersion: 1 and an '
  ...
  test('snapshot listener emits initial empty map and then updates on '
    ...
    final uid = currentUid();
    final storeReader = FirebaseConnectionStore(firestore: firestore, uid: uid);
    addTearDown(storeReader.dispose);
    final storeWriter = FirebaseConnectionStore(firestore: firestore, uid: uid);
  ...
  test(
    ...
    () async {
      ...
      final spoofStore = FirebaseConnectionStore(
        firestore: firestore,
        uid: uidA,
      );
  ...
  test(
    ...
    () async {
      ...
      final spoof = FirebaseConnectionStore(firestore: firestore, uid: uidA);
  ...
  test('connectionStoreProvider rebuilds for a new user and the new '
      'store sees an empty collection (Pass 4.5 #065 type guard)', () async {
    ...
    expect(
      storeA,
      isA<FirebaseConnectionStore>(),
      reason:
          'signed-in connectionStoreProvider must return the '
    ...
    expect(
      ...
      isFalse,
      reason:
          'connectionStoreProvider must rebuild when the auth '
    ...
    expect(
      ...
      isEmpty,
      reason:
          'user B starts with an empty connections collection; '

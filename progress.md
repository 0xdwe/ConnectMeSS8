...
**Pass 4.5 begins on 2026-05-26.** Connection / interaction / event persistence, in flight. PRD at `docs/prd/2026-05-26-connection-persistence-pass-4-5-prd.md`. Of the ten planned issues #063–#072, the gate (#063) and the five store-infrastructure issues (#064–#068) have shipped today. Remaining: #069 (seeder + onboarding), #070 (AppController write-through-store rewrite), #071 (cutover + smoke), #072 (closeout + revert sign-out hotfix). Pass 4.5 was added to the plan after grilling Pass 4.2 surfaced that only per-contact memory persists across sign-out; connections, interactions, and events still live in RAM.
**Test baseline** — `flutter test test/state/connections/`: **76 passed, 0 failed** (Pass 4.5 surface; up from 0 pre-#064). JS rules tests at `firestore/rules.test.js`: **184 passed, 0 failed** (up from 38 pre-Pass-4.5, now covering connections/interactions/events). Default `flutter test` sweep on `main` is unchanged from Pass 4.2 because Pass 4.5 has not yet flipped `AppController` over to the new stores — 273 passed, 33 failed (the 33 failures are widget-test fixture drift from the unrelated `ui-login-page` and `fix-navbar` UI merges, tracked separately). Emulator-backed `flutter test integration_test` runs separately (see `integration_test/firebase_test_setup.dart`).
...
- **Pass 4.5 — connection / interaction / event persistence.** In progress on 2026-05-26. The Pass 4.2 PRD's blind spot — "memory follows the user, not the install" was promised but only the per-contact memory side shipped — is closed by this pass. PRD covers 14 implementation decisions (Q1–Q14) post-grilling. Of the ten issues #063–#072: #063 (PRD pre-flight gate, no code), #064 (ConnectionStore seam + InMemory + auth-aware provider), #065 (FirebaseConnectionStore with snapshot listener), #066 (Firestore rules + JS tests for connections, +50 cases), #067 (InteractionStore end-to-end, +45 JS rules cases + 26 headless tests), #068 (EventStore end-to-end, +51 JS rules cases + 26 headless tests) — all merged today. Remaining: #069 (seeder + onboarding + empty-state UX), #070 (AppController write-through-store rewrite, the cutover), #071 (cutover + two-device smoke), #072 (Pass 4.5 closeout + revert sign-out hotfix). The sign-out hotfix on `main` (`4a4a7e5`) preserves user data across sign-out as a stop-gap; #072 will revert it once Firestore is the source of truth.
...
- **Pass 4.5 is mid-flight.** Six issues shipped today (#063, #064, #066, #065, #067, #068). Four to go: #069 (seeder + onboarding + empty-state UX), #070 (AppController write-through-store rewrite — the cutover), #071 (cutover + two-device smoke), #072 (closeout + revert sign-out hotfix). The store infrastructure is complete and the new auth-aware providers are wired but unused; AppController still mutates in-memory lists. #069 is the next pickup.
...
- **Sign-out hotfix on `main` (`4a4a7e5`) is intentional debt.** Preserves user data across sign-out as a stop-gap until Pass 4.5 has Firestore as the source of truth. #072 reverts it and rewrites the 5 hotfix tests in `test/state/app_state_test.dart:224-365`.
...
- **UI-merge regression on `main`.** The `ui-login-page` and `fix-navbar` merges (commits `7694253`, `88ebfcc`) brought 32 widget-test failures onto `main`. Out of scope for Pass 4.2 / Pass 4.5; deserves its own hotfix issue.

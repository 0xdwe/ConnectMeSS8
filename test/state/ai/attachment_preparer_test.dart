...
void main() {
  group('extension classification', () {
    test('treats jpg/jpeg/png/webp/heic as image extensions',
        () async {
      ...
      for (final entry in result.nameOnly) {
        expect(entry.softFailReason,
            AttachmentDegradeReason.fileNotFound);
      }
    });

    test('treats pdf, m4a, txt, and extension-less as non-image',
        () async {
      final result = await prepareAttachments(
        [
  ...
  group('happy path — decode + downscale + JPEG re-encode', () {
    ...
    test('downscales landscape image to fit 1024 on long edge',
        () async {
    ...
    test('downscales portrait image to fit 1024 on long edge',
        () async {
    ...
    test('JPEG re-encode produces meaningfully smaller bytes for big images',
        () async {
  ...
  group('per-call image cap', () {
    test('caps images at kMaxImagesPerCall and degrades overflow',
        () async {
      ...
      for (final overflow in result.nameOnly) {
        expect(overflow.softFailReason,
    ...
    test('respects custom maxImages override', () async {
      ...
      expect(result.nameOnly.single.softFailReason,
          AttachmentDegradeReason.perCallImageCap);
    });
    ...
    test(
      ...
      expect(result.images.map((i) => i.name), containsAll(['a.jpg', 'b.jpg']));
      expect(
  ...
  group('mixed input', () {
    test('image + non-image + cap overflow + soft-fail in one batch',
        () async {
      final src = _buildJpegBytes(width: 100, height: 100);
      final attachments = <AttachmentRef>[
        const AttachmentRef(name: 'a.jpg', path: '/tmp/a.jpg'),
      ...
      final result =
          await prepareAttachments(attachments, reader: reader);

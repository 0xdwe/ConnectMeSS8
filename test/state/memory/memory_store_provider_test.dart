...
void main() {
  ...
  group('memoryStoreProvider — signed in', () {
    ...
    test(
      'currentUserProvider goes null on auth swap to a signed-out mock',
      () async {
        final authA = MockFirebaseAuth(
          signedIn: true,
          mockUser: MockUser(uid: 'user-a', isAnonymous: false),
  ...
  group('memoryStoreProvider — direct override (existing test pattern)', () {
    test(
      ...
      () async {
        ...
        final container = ProviderContainer(
          overrides: [memoryStoreProvider.overrideWithValue(injected)],
        );
        addTearDown(container.dispose);
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          'Account B note',
          reason:
              'memoryProvider must read through the new store '

...
void main() {
  ...
  group('interactionStoreProvider — signed out', () {
    test('returns a sentinel whose async surface throws StateError', () async {
      ...
      await expectLater(store.load('i1'), throwsA(isA<StateError>()));
      await expectLater(
        store.save(makeInteraction('i1')),
        throwsA(isA<StateError>()),
      );
      await expectLater(store.delete('i1'), throwsA(isA<StateError>()));
  ...
  group('interactionStoreProvider — signed in', () {
    ...
    test('currentUserProvider goes null on auth swap to a signed-out '
        'mock; interactionStoreProvider falls back to the sentinel', () async {
      final authA = MockFirebaseAuth(
        signedIn: true,
        mockUser: MockUser(uid: 'user-a', isAnonymous: false),
      );
  ...
  group(
    'interactionStoreProvider — direct override (existing test pattern)',
    () {
      test(
        ...
        () async {
          ...
          final container = ProviderContainer(
            overrides: [interactionStoreProvider.overrideWithValue(injected)],
          );
          addTearDown(container.dispose);

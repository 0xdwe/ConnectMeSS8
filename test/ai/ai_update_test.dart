...
void main() {
  group('MockAiUpdate.run', () {
    test(
      'produces an interaction with the categorized type and AI source',
      () async {
    ...
    test(
      'two runs with the same input produce structurally identical output',
      () async {
        final container = _container();
        addTearDown(container.dispose);
    ...
    test(
      'a second run on a memory with prior history appends rather than overwrites',
      () async {
        final container = _container();
        addTearDown(container.dispose);
    ...
    test(
      'preview-then-commit two-step persists memory and applies state delta',
      () async {
    ...
    test(
      'cancel — run produces a result, commit is never called, store is untouched',
      () async {
        final store = InMemoryMemoryStore();
        ...
        final beforeInteractions = container
            ...
            .length;
        final mike = _connection(container.read(appControllerProvider), 'mike');
        final beforeMemory = await container.read(
          memoryProvider('mike').future,
        );
        ...
        expect(
          ...
          contains('This will be cancelled.'),
        );
  ...
  group('AiUpdate all-or-nothing (PRD Q4 / #046)', () {
    test(
      'failOnRun: nothing persists, both sides observe the pre-run state',
      () async {
        final store = InMemoryMemoryStore();
    ...
    test(
      'failOnSave: state delta is never applied; memory unchanged',
      () async {
        final store = InMemoryMemoryStore();
        final container = ProviderContainer(
          overrides: [
    ...
    test('failOnApply: memory file is rolled back to the pre-run state '
  ...
  group('MockAiUpdate.run populates bondScoreDelta via the curve '
      '(#085 Slice 4c)', () {
    test(
      'Mike at bond 68 with depth=50 → +10 (floor(50 × 32 / 160))',
      () async {
        final container = _container();
        addTearDown(container.dispose);
        final mike = _connection(container.read(appControllerProvider), 'mike');
    ...
    test('Sarah at bond 92 with depth=50 → +2 (floor(50 × 8 / 160))', () async {
      final container = _container();
      addTearDown(container.dispose);
      final sarah = _connection(container.read(appControllerProvider), 'sarah');
      final memory = await container.read(memoryProvider('sarah').future);

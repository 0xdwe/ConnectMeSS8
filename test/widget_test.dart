...
void main() {
  testWidgets('auth flow enters app and bottom nav switches tabs', (tester) async {
  ...
  testWidgets('plus menu add connection mutates visible people list', (tester) async {
    await pumpConnectMe(tester);
    await tester.tap(find.byKey(const Key('sign-in-button')));
    await tester.pumpAndSettle();
    ...
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const Key('add-name-field')), 'Ari Rivera');
    await tester.enterText(find.byKey(const Key('add-email-field')), 'ari@email.com');
  ...
  testWidgets('plus menu update connection opens AI update page', (tester) async {
    await pumpConnectMe(tester);
    await tester.tap(find.byKey(const Key('sign-in-button')));
    await tester.pumpAndSettle();
    ...
    await tester.pumpAndSettle();
    await tester.scrollUntilVisible(find.text('Mike Chen'), 120, scrollable: find.byType(Scrollable).last);
